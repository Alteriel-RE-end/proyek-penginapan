...
// =================================================================
// == BAGIAN 4: LOGIKA DASHBOARD (ADMIN)
// =================================================================

if (window.location.pathname.includes("dashboard.html")) {
    
    // --- KONSTANTA DEVICE & REFERENSI UMUM ---
    ...
    // --- Data Kustom Relay (Dummy Awal, akan disinkronisasi ke Firestore nanti)
    let relayNames = {
        'RELAY1': 'Lampu Kamar Tidur',
        'RELAY2': 'Lampu Kamar Mandi',
        'RELAY3': 'Kipas',
        'RELAY4': 'Lampu Teras'
    };
    let relayStates = { 'RELAY1': 'OFF', 'RELAY2': 'OFF', 'RELAY3': 'OFF', 'RELAY4': 'OFF' };

    // --- Fungsi Helper MQ-135 ---
    ...
    // =================================================================
    // == BAGIAN 5: LOGIKA KONTROL & GRAFIK (KAMAR 1 & UMUM)
    // =================================================================

    // --- Fungsi 1: Gambar Kontrol Relay ---
        ...
        for (let i = 1; i <= 4; i++) {
            ...
            html += `
                <div class="relay-item">
                    ...
                    <span class="status-indicator ${colorClass}">${state}</span>
                </div>
            `;
        }
        html += '</div>';
        kontrolDiv.innerHTML = html;

        kontrolDiv.querySelectorAll('.btn-on, .btn-off').forEach(btn => {
            btn.addEventListener('click', handleRelayControl);
        });
    }

    // --- Fungsi 2: Kirim Perintah Kontrol Relay (VIA API Vercel) ---
    async function handleRelayControl(e) {
        ...
        console.log(`Perintah Admin: Mengirim ${command} ke Kamar 1...`);

        try {
            const response = await fetch('/api/kontrolRelay', {
                method: 'POST',
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Gagal mengirim perintah ke server.');
            }
            
            // Update tampilan setelah sukses
            relayStates[relayId] = newState;
            renderRelayControls();

        } catch (error) {
            console.error('Kontrol Gagal:', error);
            alert(`Gagal Kontrol Relay: ${error.message}`);
        }
    }

    // --- Fungsi 3: Struktur Pengaturan Nama Relay ---
        ...
        for (let i = 1; i <= 4; i++) {
            ...
            html += `
                <div class="form-group">
                    <label>Relay ${i} (UID: ${relayId}):</label>
                    <input type="text" id="name-${relayId}" value="${name}" required>
                </div>
            `;
        }
    ...
    // --- LOGIKA EVENT LISTENER UNTUK RESOLUSI ---
    ...
    // Tambahkan Panggilan setupChartListeners() ke dalam blok di bawah
    if (window.location.pathname.includes("dashboard.html")) {
        // ... (Kode yang sudah ada) ...
        // Panggil fungsi utama saat halaman detail dibuka (di event listener yang sudah ada)
...
} // <-- Penutup if dashboard.html yang benar

// =================================================================
// == BAGIAN 6: LOGIKA HALAMAN PUBLIK (INDEX.HTML)
// =================================================================

// ... (Kode BAGIAN 5 sama seperti sebelumnya untuk Halaman Publik) ...

// Ini adalah duplikasi dari kode di atas untuk memastikan semua fungsi terpakai
